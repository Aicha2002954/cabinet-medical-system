import React, { useState, useEffect } from "react";
import axios from "axios";
import { FaEdit, FaTrash, FaEye, FaTimes, FaUserSecret, FaSpinner, FaSearch, FaUserPlus, FaPhone, FaEnvelope, FaMapMarkerAlt, FaIdCard, FaFileExcel, FaFilePdf, FaChartLine } from "react-icons/fa";
import * as XLSX from "xlsx";
import jsPDF from "jspdf";
import "jspdf-autotable";

const SecretaireTab = ({ onSecretaireUpdate }) => {
    const [secretaires, setSecretaires] = useState([]);
    const [showModal, setShowModal] = useState(false);
    const [isEditing, setIsEditing] = useState(false);
    const [formData, setFormData] = useState({
        userId: "", firstName: "", lastName: "", email: "", phone: "", cni: "", address: "", password: "", confirmPassword: ""
    });
    const [loading, setLoading] = useState(false);
    const [searchTerm, setSearchTerm] = useState("");
    const [currentPage, setCurrentPage] = useState(1);
    const itemsPerPage = 8;
    const [stats, setStats] = useState({ total: 0, newThisMonth: 0 });

    const API_URL = "http://localhost:8087/api/profiles";
    const getAuthHeader = () => ({
        Authorization: `Bearer ${localStorage.getItem("token")}`
    });

    const fetchSecretaires = async () => {
        try {
            const res = await axios.get(API_URL, { headers: getAuthHeader() });
            const allUsers = res.data;
            const secretairesList = allUsers.filter(user => user.role === "SECRETAIRE");
            setSecretaires(secretairesList);
            if (onSecretaireUpdate) onSecretaireUpdate(secretairesList.length);
            setStats({ total: secretairesList.length, newThisMonth: 0 });
        } catch (err) {
            console.error("Erreur chargement secrétaires :", err);
            alert("Impossible de charger les secrétaires.");
        }
    };

    useEffect(() => { fetchSecretaires(); }, []);

    const filteredSecretaires = secretaires.filter(s =>
        s.firstName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        s.lastName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        s.email?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        (s.cni || "").toLowerCase().includes(searchTerm.toLowerCase()) ||
        (s.address || "").toLowerCase().includes(searchTerm.toLowerCase())
    );

    const totalPages = Math.ceil(filteredSecretaires.length / itemsPerPage);
    const currentSecretaires = filteredSecretaires.slice((currentPage-1)*itemsPerPage, currentPage*itemsPerPage);

    const handleInputChange = (e) => setFormData({ ...formData, [e.target.name]: e.target.value });
    const openModal = (secretaire = null) => {
        if (secretaire) {
            setIsEditing(true);
            setFormData({
                userId: secretaire.userId,
                firstName: secretaire.firstName || "",
                lastName: secretaire.lastName || "",
                email: secretaire.email || "",
                phone: secretaire.phone || "",
                cni: secretaire.cni || "",
                address: secretaire.address || "",
                password: "",
                confirmPassword: ""
            });
        } else {
            setIsEditing(false);
            setFormData({ userId: "", firstName: "", lastName: "", email: "", phone: "", cni: "", address: "", password: "", confirmPassword: "" });
        }
        setShowModal(true);
    };
    const closeModal = () => setShowModal(false);

    const handleSave = async (e) => {
        e.preventDefault();
        if (!isEditing && (!formData.password || formData.password !== formData.confirmPassword)) {
            alert("Mots de passe différents ou manquant");
            return;
        }
        setLoading(true);
        try {
            if (isEditing) {
                if (!formData.userId) {
                    alert("ID du secrétaire manquant");
                    return;
                }
                const payload = new FormData();
                payload.append("firstName", formData.firstName || "");
                payload.append("lastName", formData.lastName || "");
                payload.append("phone", formData.phone || "");
                payload.append("cni", formData.cni || "");
                payload.append("address", formData.address || "");
                payload.append("email", formData.email || "");
                await axios.put(`${API_URL}/${formData.userId}`, payload, {
                    headers: getAuthHeader()
                });
                alert("Secrétaire modifié ✅");
            } else {
                const { userId, confirmPassword, ...payload } = formData;
                payload.role = "SECRETAIRE";
                if (!payload.password) {
                    alert("Mot de passe requis");
                    return;
                }
                Object.keys(payload).forEach(key => {
                    if (payload[key] === "") delete payload[key];
                });
                await axios.post(API_URL, payload, {
                    headers: { ...getAuthHeader(), 'Content-Type': 'application/json' }
                });
                alert("Secrétaire ajouté ✅");
            }
            closeModal();
            fetchSecretaires();
        } catch (err) {
            console.error(err);
            alert("Erreur: " + (err.response?.data?.message || "Vérifiez la console"));
        } finally {
            setLoading(false);
        }
    };

    const handleDelete = async (userId) => {
        if (!userId) {
            alert("ID invalide");
            return;
        }
        if (window.confirm("Supprimer ce secrétaire ?")) {
            try {
                await axios.delete(`${API_URL}/${userId}`, { headers: getAuthHeader() });
                fetchSecretaires();
            } catch (err) {
                alert("Erreur lors de la suppression");
            }
        }
    };
    const showDetails = (s) => alert(`Détails:\nNom: ${s.firstName} ${s.lastName}\nEmail: ${s.email}\nTél: ${s.phone}\nCNI: ${s.cni || "—"}\nAdresse: ${s.address || "—"}`);

    const exportToExcel = () => {
        const data = filteredSecretaires.map(s => ({
            "Nom complet": `${s.firstName} ${s.lastName}`,
            Email: s.email,
            Téléphone: s.phone,
            CNI: s.cni || "",
            Adresse: s.address || ""
        }));
        const ws = XLSX.utils.json_to_sheet(data);
        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, "Secretaires");
        XLSX.writeFile(wb, `secretaires_${new Date().toISOString().slice(0,19)}.xlsx`);
    };

    const exportToPDF = () => {
        const doc = new jsPDF();
        doc.setFontSize(16);
        doc.text("Liste des secrétaires", 14, 15);
        doc.setFontSize(10);
        doc.text(`Généré le ${new Date().toLocaleString()}`, 14, 22);
        const tableData = filteredSecretaires.map(s => [
            `${s.firstName} ${s.lastName}`,
            s.email,
            s.phone,
            s.cni || "",
            s.address || ""
        ]);
        doc.autoTable({
            head: [["Nom complet", "Email", "Téléphone", "CNI", "Adresse"]],
            body: tableData, startY: 30, theme: "striped", styles: { fontSize: 8 }, headStyles: { fillColor: [44,125,160], textColor: 255 }
        });
        doc.save(`secretaires_${new Date().toISOString().slice(0,19)}.pdf`);
    };

    return (
        <div className="premium-patients-container">
            <div className="stats-premium-row">
                <div className="stat-premium-card"><FaUserSecret /> <span>{stats.total}</span> <label>Total secrétaires</label></div>
                <div className="stat-premium-card"><FaChartLine /> <span>{stats.newThisMonth}</span> <label>Nouveaux ce mois</label></div>
           
            </div>
            <div className="toolbar-premium">
                <div className="search-premium"><FaSearch /><input type="text" placeholder="Rechercher..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} /></div>
                <div className="actions-premium">
                    <button className="btn-excel" onClick={exportToExcel}><FaFileExcel /> Excel</button>
                    <button className="btn-pdf" onClick={exportToPDF}><FaFilePdf /> PDF</button>
                    <button className="btn-add-premium" onClick={() => openModal()}><FaUserPlus /> Nouveau secrétaire</button>
                </div>
            </div>
            <div className="luxury-table-wrapper">
                <table className="luxury-table">
                    <thead>
                        <tr>
                            <th>Secrétaire</th>
                            <th>Contact</th>
                            <th>Adresse</th>
                            <th>Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        {currentSecretaires.map(s => (
                            <tr key={s.userId}>
                                <td className="patient-cell">
                                    <div className="avatar-small"><FaUserSecret /></div>
                                    <div><strong>{s.firstName} {s.lastName}</strong></div>
                                </td>
                                <td>{s.phone}<br/><small>{s.email}</small></td>
                                <td>{s.address || "—"}</td>
                                <td className="action-icons">
                                    <button className="icon-btn view" onClick={() => showDetails(s)}><FaEye /></button>
                                    <button className="icon-btn edit" onClick={() => openModal(s)}><FaEdit /></button>
                                    <button className="icon-btn delete" onClick={() => handleDelete(s.userId)}><FaTrash /></button>
                                </td>
                            </tr>
                        ))}
                        {currentSecretaires.length === 0 && (
                            <tr key="empty-row">
                                <td colSpan="5" style={{textAlign:"center"}}>Aucun secrétaire trouvé</td>
                            </tr>
                        )}
                    </tbody>
                </table>
            </div>
            {totalPages > 1 && (
                <div className="pagination-premium">
                    <button disabled={currentPage===1} onClick={()=>setCurrentPage(p=>p-1)}>◀ Précédent</button>
                    <span>Page {currentPage} / {totalPages}</span>
                    <button disabled={currentPage===totalPages} onClick={()=>setCurrentPage(p=>p+1)}>Suivant ▶</button>
                </div>
            )}
            {showModal && (
                <div className="modal-premium" onClick={closeModal}>
                    <div className="modal-premium-content" onClick={e=>e.stopPropagation()}>
                        <div className="modal-premium-header">
                            <h3>{isEditing ? "Modifier secrétaire" : "Ajouter secrétaire"}</h3>
                            <FaTimes className="close-premium" onClick={closeModal} />
                        </div>
                        <form onSubmit={handleSave}>
                            <div className="form-grid">
                                <input name="firstName" placeholder="Prénom" value={formData.firstName} onChange={handleInputChange} required />
                                <input name="lastName" placeholder="Nom" value={formData.lastName} onChange={handleInputChange} required />
                                <input name="phone" placeholder="Téléphone" value={formData.phone} onChange={handleInputChange} required />
                                <input name="cni" placeholder="CNI" value={formData.cni} onChange={handleInputChange} />
                                <input name="address" placeholder="Adresse" value={formData.address} onChange={handleInputChange} />
                                <input name="email" placeholder="Email" type="email" value={formData.email} onChange={handleInputChange} required disabled={isEditing} />
                                {!isEditing && (
                                    <>
                                        <input name="password" type="password" placeholder="Mot de passe" onChange={handleInputChange} required />
                                        <input name="confirmPassword" type="password" placeholder="Confirmer" onChange={handleInputChange} required />
                                    </>
                                )}
                            </div>
                            <button type="submit" className="save-premium" disabled={loading}>
                                {loading ? <FaSpinner className="spin" /> : "Enregistrer"}
                            </button>
                        </form>
                    </div>
                </div>
            )}
        </div>
    );
};

export default SecretaireTab;